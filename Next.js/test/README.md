...
### 4.20 다중 서명 지갑 (MultiSig Wallet) 실습 정리

개인 지갑의 프라이빗 키 유출에 따른 해킹 위험을 원천 차단하기 위해, 거래소나 기업, DAO 등에서 사용하는 필수 금고형 스마트 컨트랙트입니다.

#### 📋 구현 파일 목록
| 파일 | 역할 |
|---|---|
| contracts/MultiSigWallet.sol | 소유자 지정, 송금 기안(Submit), 결재(Confirm), 최종 실행(Execute) 로직 구현 |
| scripts/deploy-multisig.js | 로컬 계정 3개를 소유자로 지정하고, '3명 중 2명' 결재 조건(Threshold)으로 금고 배포 |
| app/multisig/page.tsx | 송금 안건 등록, 결재 진행 상황 확인 및 서명 승인을 위한 대시보드 |

#### 💡 핵심 인사이트
* **보안의 혁신**: 지갑 주인의 프라이빗 키 하나가 해킹으로 털려도, 지정된 결재 기준(Threshold)을 만족하지 못하면 해커는 절대 금고 안의 돈을 빼낼 수 없습니다.
* **트랜잭션 생명주기**: 개인 지갑은 [서명 -> 송금]으로 끝나지만, MultiSig는 **[안건 기안 -> 다수결 결재 도장 모으기 -> 조건 달성 시 실제 송금 실행]** 이라는 단계적 승인 절차를 거칩니다.

---

### 4.21 토큰 베스팅 (Token Vesting) 실습 정리

주식의 '보호 예수'나 스타트업의 '스톡옵션 베스팅'과 동일한 개념으로, 개발팀이나 초기 투자자의 코인 물량을 스마트 컨트랙트에 묶어두고 서서히 푸는 시스템입니다. (Rug Pull, 먹튀 방지용)

#### 📋 구현 파일 목록
| 파일 | 역할 |
|---|---|
| contracts/TokenVesting.sol | 절벽(Cliff)과 선형 분배(Linear Vesting) 로직이 구현된 락업 금고 |
| scripts/deploy-vesting.js | 팀 토큰(TokenA)을 배포하고 50만 개를 베스팅 금고로 전송해 락업 |
| app/vesting/page.tsx | 타임라인 시각화 및 실시간으로 초당 풀리는 토큰을 수령(Claim)하는 UI |

#### 💡 핵심 인사이트
* **신뢰를 코드로 강제 (Trustless)**: "우리 팀은 토큰 안 팔고 3년 동안 열심히 개발하겠습니다!"라는 말 대신, **"우리 팀 물량은 블록체인 코드에 의해 3년 동안 묶여서 아예 팔고 싶어도 못 팝니다"** 라고 증명하는 것이 웹3의 철학입니다.
* **Cliff (절벽)**: 락업 시작 후, 토큰이 단 1개도 풀리지 않는 절대 금지 기간입니다. (보통 상장 후 6개월~1년)
* **Vesting (분배)**: 절벽 기간이 지나면 한 번에 다 주는 것이 아니라, 수학 공식(전체 기간 대비 경과 시간 비율)에 의해 초 단위로 물량이 서서히 분배됩니다.

---

### 4.22 디파이 대출 (Lending Protocol) 및 청산(Liquidation) 개념

Aave, MakerDAO 같은 디파이(DeFi) 생태계의 심장 역할을 하는 스마트 컨트랙트입니다. 블록체인에는 현실 은행 같은 신용 점수가 없기 때문에, 전당포처럼 반드시 내 자산을 먼저 담보로 맡겨야만 다른 코인을 빌릴 수 있는 **초과 담보 대출(Over-collateralization)** 방식을 사용합니다.

#### 💡 핵심 인사이트
* **초과 담보 대출**: 100만 원어치 달러 코인을 빌리려면, 무조건 그보다 비싼 150만 원어치의 이더리움(ETH)을 담보로 맡겨야 합니다. (신용 대출 절대 불가)
* **오라클(Oracle) 연동**: 담보물(ETH)의 가치가 시시각각 변하기 때문에, 이전에 실습했던 Chainlink 오라클 가격 피드를 실시간으로 감시하여 담보물의 현재 가치를 평가합니다.
* **강제 청산 (Liquidation)**: 만약 ETH 가격이 폭락하여 담보물의 가치가 빌려간 돈의 가치와 비슷해질 위험(LTV 초과)이 생기면, 시스템이 손해를 보기 전에 컨트랙트가 담보물을 시장에 강제로 팔아버려 빚을 회수합니다. 이것이 디파이 은행들이 파산하지 않고 굴러가는 핵심 안정성 메커니즘입니다.
